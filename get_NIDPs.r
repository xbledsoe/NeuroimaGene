...
def='/gpfs52/data/g_gamazon_lab/xbledsoe/PhD-work/NeuroimaGENE/Analysis/TWAS_33K/all_assocs_33K_'
...
option_list = list(
    ...
    make_option(c("-m", "--modality"), type="character", default= def, 
              help="NIDP modality of interest", metavar="character"),    
    make_option(c("-a", "--atlas"), type="character", default= def, 
              help="NIDP atlas of interest", metavar="character"),
    make_option(c("-p", "--mtc"), type="character", default= def, 
              help="pvalue threshold", metavar="character"),
    make_option(c("-t", "--typ"), type="character", default= def, 
              help="type of analysis (modality, atlas, all)", metavar="character"),
    make_option(c("-s", "--short_name"), type="character", default= def, 
...
colnm = paste0(opt$typ, '_', opt$mtc, 'pval')
}
...
sqlcmd = paste0( "SELECT gene, gene_name, zscore, gwas_phenotype, training_model, ", 
                            colnm, 
                        " FROM associations WHERE gene IN (", paste(shQuote(gns$V1, type = "sh"), collapse = ', '), 
                            ") OR gene_name IN (", paste(shQuote(gns$V1, type = "sh"), collapse = ', '),
                        ") AND ", colnm, "< 0.05 AND atlas='", opt$atlas, "' AND modality='", opt$modality,"'")
